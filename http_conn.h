/* 需要处理的逻辑任务对象通过http_conn类实例化，放入请求队列 */
...
class http_conn {
public:
    static const int FILENAME_LEN = 200;    // 文件名最大长度，整个类都一样，所以用static
...
private:
    ...
    bool add_status_line(int status, const char *line);
    bool add_headers(int content_length);
    bool add_content_length(int content_lenngth);
    bool add_linger();
    bool add_blank_line();
...
private:
    ...
    int m_checked_idx; // 当前正在分析的字符再读缓冲区中的位置
    int m_start_line;   // 当前正在解析的行的起始位置
    char m_write_buf[WRITE_BUFFER_SIZE];    // 写缓冲区
    int m_write_idx;    //写缓冲区中待发送的字节数，写缓冲区存的是报头
    ...
    char *m_version;    // HTTP协议版本号，我们仅支持 HTTP/1.1 */
    char *m_host;   // 主机名
    int m_content_length;   // HTTP请求的消息体的长度
    int m_linger;   // HTTP请求是否要保持连接

    char *m_file_address;   // 客户请求的目标文件被 mmap 到内存中的起始位置
    /* 目标文件的状态，通过它我们可以判断文件是否存在，是否为目录，是否可读，并获取文件大小等信息 */
    struct stat m_file_stat;
    /* 采用writev来执行写操作，所以需要定义以下两个成员，其中m_iv_count表示被写的内存块的数量 */
    struct iovec m_iv[2];   // 头部和内容 分散写
    int m_iv_count;

    /* 修改，给客户端发送文件，写文件时使用 */
    int m_bytes_to_send;    // 写缓冲区中待发送的字节
    int m_bytes_have_send;  // 写缓冲区中已发送的字节
};
